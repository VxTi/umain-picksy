...
Ditto store observers (`install_photos_observer` in `src-tauri/src/ditto_repo.rs`) run inline with the store thread. Doing heavy work (deserializing large payloads or emitting to the UI) directly in the observer can block write transactions and make imports appear to “hang”. Also, DQL does not accept `DOCUMENTS (:docs)` with an array; it must insert one doc at a time (or named documents, not an array).
